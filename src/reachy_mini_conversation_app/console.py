...
This module provides the local audio streaming interface using fully local
endpoints for VAD, ASR, LLM, and TTS. The settings UI allows users to
configure personalities without needing the full Gradio interface.
"""
...
class LocalStream:
    ...
    def _init_settings_ui_if_needed(self) -> None:
        ...
        Mounts the personality configuration UI when a settings_app is provided.
        """
        ...
        # GET /status -> always ready (no API key needed for local mode)
        @self._settings_app.get("/status")
        def _status() -> JSONResponse:
            return JSONResponse({"has_key": True})  # Always ready in local mode
    ...
    def launch(self) -> None:
        ...
        # Expose settings UI if a settings app is available
